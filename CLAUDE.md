...
`compile_commands.json` at the repo root drives clangd. It is generated by
`bear`, gitignored, and machine-specific (absolute paths). With it present,
clangd resolves `src/`, `tests/`, and the fuzz harnesses cleanly; without it,
quoted includes fail and cascade into hundreds of bogus unknown-type errors.

Regenerate with `make compiledb`. A `PostToolUse` hook in
`.claude/settings.json` runs that target automatically whenever the `Makefile`
is edited, so a flag change cannot leave a stale database. Full regeneration
takes roughly 25 seconds and runs asynchronously.

Two traps, both hit in practice:

- **`bear` records only what actually compiles.** Running it over an up-to-date
  tree produces a partial database that still looks valid — the file parses,
  clangd loads it, and half the tree silently stops resolving. `make compiledb`
  therefore always runs `make clean` first. Never regenerate incrementally.
- **`bear -- make` alone covers only `all`.** The test sources build from
  separate targets and need `-Itests`; omit them and every file under `tests/`
  fails on `test_assert.h`. The target builds `all` plus `$(TEST_BINS)`.

Do not add a `.clangd` file. Without a database, clangd's fallback command runs
with the working directory set to the edited file's own directory, so relative
`-I` paths silently miss; the database supersedes it entirely.
