...
```
@source = "hello = {@name} code ( if !@name then @name = \"Unknown User\"; endif; sys.log{\"Hello \"); sys.log{@name}; sys.log{\"!\n\"}; );";
@result = sys.compile{@source};
hello{"Boris"};
hello;
```

This code will output `"Hello Boris!"` followed by `"Hello Unknown User!"`.  Note that if a code item expects a parameter but does not receive one, it is initialised to `nil`.
