...
Triple-quoted strings are raw: their contents are passed through unchanged, so
they are particularly convenient for source supplied to `sys.compile`.  The
`\n` sequences above are therefore interpreted by the compilation performed by
`sys.compile`, rather than by the string literal which contains the source.
