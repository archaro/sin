...
| --- | --- | --- | --- |
| `a` | `IR_OP_ADD` | none | Pop the top two values. Add integers, concatenate strings, and push the result. Invalid mixed types push `nil`. |
...
| `d` | `IR_OP_DIV` | none | Pop two integers, divide the previous value by the top value, and push the result. Invalid operands produce `nil`; divide by zero substitutes zero. |
...
| `m` | `IR_OP_MUL` | none | Pop two integers, multiply them, and push the result. Invalid operands produce `nil`. |
| `n` | `IR_OP_NEG` | none | Pop an integer, negate it, and push it back. |
...
| `s` | `IR_OP_SUB` | none | Pop two integers, subtract the top value from the previous value, and push the result. Invalid operands produce `nil`. |
