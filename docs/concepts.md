...
`net.ditch{<integer>}` marks an active line for disconnection.  It returns `true` when the line is active and has been marked, `false` when the line is out of range or already inactive, and `nil` when the argument is not a non-negative integer.  Invalid arguments set `ERR_RUNTIME_INVALIDARGS`; out-of-range or inactive-line failures set the network error item.
