...
`net.flush{<integer>}` requests an immediate flush of pending output for an active line.  It returns `true` when the line is active, `false` when the line is out of range or inactive, and `nil` when the argument is not a non-negative integer.  The `nil` case does not set `error`; out-of-range or inactive-line failures set the network error item.
`net.ditch{<integer>}` marks an active line for disconnection.  It returns `true` when the line is active and has been marked, `false` when the line is out of range or already inactive, and `nil` when the argument is not a non-negative integer.  The `nil` case does not set `error`; out-of-range or inactive-line failures set the network error item.
