...
Relevant build and test targets are `make`, `make lib`, `make test`,
`make bench`, `make test-sanitize`, `make test-fuzz`, and `make test-full`.
Relevant variables are `BUILD=debug|release|sanitize|coverage`, `CC=gcc|clang`,
