...
TEST_SOURCES := $(wildcard $(FRAMEWORK_DIR)/*.c $(CONFORMANCE_DIR)/*.c \
 ...
 $(TEST_DIR)/interpreter/*.c $(TEST_DIR)/adapters/*.c \
 $(TEST_DIR)/adapters/group1/*.c $(TEST_DIR)/network/*.c)
TEST_OBJECTS := $(foreach source,$(TEST_SOURCES),$(call test_object,$(source)))
ADAPTER_COMMON_SOURCES := $(FRAMEWORK_SOURCES) $(TEST_DIR)/shared/test_helpers.c \
 $(TEST_DIR)/shared/test_libcall_support.c $(TEST_DIR)/shared/test_pipeline_cases.c
ADAPTER_COMMON := $(foreach source,$(ADAPTER_COMMON_SOURCES),$(call test_object,$(source)))
TEST_PROGRAMS := $(VARIANT_PROGRAMS)

ADAPTER_GROUP1_BINS := $(addprefix $(OBJ_DIR)/$(ADAPTER_DIR)/group1/test_,absyn_lifecycle parser_input_api cli_io parser_float_literals sconv value_behavior libcall_registry libcall_sys fixture_policy output_contract memory)
ADAPTER_GROUP2_BINS := $(addprefix $(OBJ_DIR)/$(ADAPTER_DIR)/,test_semant test_ir_validate test_relative_item_leading_dot test_pipeline_golden test_pipeline_source_golden test_pipeline_negative_matrix test_parser_examples_obj_golden test_compiler_context_failures test_compiler_diag_pipeline test_opcode_schema test_bytecode_convert test_bytecode_v1_abi test_bytecode_verify test_bytecode_wire test_emitbc_all_ir_ops test_emitbc_header test_emitbc_invariants test_emitbc_jumps test_emitbc_opcode_map test_emitbc_post_verify test_sdiss_fixtures test_stack_frames test_list test_interpret_semantics test_interpret_stress test_runtime_benchmark test_item_cache test_itemstore_io test_sin_itemstore_policy test_libcall_task test_task_lifecycle test_libcall_net test_libcall_str test_libcall_list test_libcall_math test_libcall_rand test_libcall_time test_libcall_conv test_libcall_text test_libcall_sys_compile test_network test_chat_smoke test_cli_contract_matrix)
ADAPTER_BINS := $(ADAPTER_GROUP1_BINS) $(ADAPTER_GROUP2_BINS)
TEST_BINS := $(FRAMEWORK_SELF_BIN) $(FRAMEWORK_RUNNER_BIN) $(FRAMEWORK_DUP_BIN) $(FRAMEWORK_NEG_BIN) $(CONFORMANCE_BIN) $(ADAPTER_BINS)
...
ADAPTER_COMMON_CASES := \
 ...
 test_libcall_conv|group6_adapter_libcall_conv.c|core/test_libcall_conv.c \
 test_libcall_text|group6_adapter_libcall_text.c|core/test_libcall_text.c

define adapter_common_template
$(OBJ_DIR)/$(ADAPTER_DIR)/$(1): $(ADAPTER_COMMON) $(call test_object,$(ADAPTER_DIR)/$(2)) $(call test_object,$(TEST_DIR)/$(3)) $(LIB) $(TEST_PROGRAMS)
	@mkdir -p $$(@D)
	$(CC) -o $$@ $(ADAPTER_COMMON) $(call test_object,$(ADAPTER_DIR)/$(2)) $(call test_object,$(TEST_DIR)/$(3)) $(LIB) $$(LDFLAGS) $$(LIBS)
endef
$(foreach c,$(ADAPTER_COMMON_CASES),$(eval $(call adapter_common_template,$(word 1,$(subst |, ,$(c))),$(word 2,$(subst |, ,$(c))),$(word 3,$(subst |, ,$(c))))))

$(OBJ_DIR)/$(ADAPTER_DIR)/test_item_cache: LDFLAGS += -Wl,--wrap=calloc
$(OBJ_DIR)/$(ADAPTER_DIR)/test_libcall_conv: LDFLAGS += -Wl,--wrap=strlen
$(OBJ_DIR)/$(ADAPTER_DIR)/test_libcall_time: private LDFLAGS += -Wl,--wrap=gmtime_r

ADAPTER_FRAMEWORK_ONLY_CASES := \
...
define adapter_framework_only_template
$(OBJ_DIR)/$(ADAPTER_DIR)/$(1): $(foreach source,$(FRAMEWORK_SOURCES),$(call test_object,$(source))) $(FRAMEWORK_HEADERS) $(call test_object,$(TEST_DIR)/shared/test_helpers.c) $(call test_object,$(ADAPTER_DIR)/$(2)) $(LIB) $(TEST_PROGRAMS)
	@mkdir -p $$(@D)
	$(CC) -o $$@ $(foreach source,$(FRAMEWORK_SOURCES),$(call test_object,$(source))) $(call test_object,$(TEST_DIR)/shared/test_helpers.c) $(call test_object,$(ADAPTER_DIR)/$(2)) $(LIB) $$(LDFLAGS) $$(LIBS)
endef
$(foreach c,$(ADAPTER_FRAMEWORK_ONLY_CASES),$(eval $(call adapter_framework_only_template,$(word 1,$(subst |, ,$(c))),$(word 2,$(subst |, ,$(c))))))

$(OBJ_DIR)/$(ADAPTER_DIR)/group1/test_fixture_policy: $(ADAPTER_COMMON) $(call test_object,$(ADAPTER_DIR)/group1/adapter_fixture_policy.c) $(call test_object,$(TEST_DIR)/shared/test_fixture_policy.c) $(LIB) $(TEST_PROGRAMS)
	@mkdir -p $(@D)
	$(CC) -o $@ $(ADAPTER_COMMON) $(call test_object,$(ADAPTER_DIR)/group1/adapter_fixture_policy.c) $(call test_object,$(TEST_DIR)/shared/test_fixture_policy.c) $(LIB) $(LDFLAGS) $(LIBS)

$(OBJ_DIR)/$(ADAPTER_DIR)/test_libcall_sys_compile: $(call test_object,$(FRAMEWORK_DIR)/test_framework.c) $(call test_object,$(TEST_DIR)/shared/test_helpers.c) $(call test_object,$(ADAPTER_DIR)/group6_adapter_libcall_sys_compile.c) $(call test_object,$(TEST_DIR)/core/test_libcall_sys_compile.c) $(LIB) $(TEST_PROGRAMS)
	@mkdir -p $(@D)
	$(CC) -o $@ $(call test_object,$(FRAMEWORK_DIR)/test_framework.c) $(call test_object,$(TEST_DIR)/shared/test_helpers.c) $(call test_object,$(ADAPTER_DIR)/group6_adapter_libcall_sys_compile.c) $(call test_object,$(TEST_DIR)/core/test_libcall_sys_compile.c) $(LIB) $(LDFLAGS) $(LIBS)

$(OBJ_DIR)/$(ADAPTER_DIR)/test_network: $(call test_object,$(FRAMEWORK_DIR)/test_framework.c) $(call test_object,$(ADAPTER_DIR)/group7_adapter_network.c)
	@mkdir -p $(@D)
	$(CC) -o $@ $(call test_object,$(FRAMEWORK_DIR)/test_framework.c) $(call test_object,$(ADAPTER_DIR)/group7_adapter_network.c) $(LDFLAGS) $(LIBS)
$(OBJ_DIR)/$(ADAPTER_DIR)/test_chat_smoke: $(foreach source,$(FRAMEWORK_SOURCES),$(call test_object,$(source))) $(call test_object,$(TEST_DIR)/shared/test_helpers.c) $(call test_object,$(ADAPTER_DIR)/group7_adapter_chat_smoke.c) $(LIB) $(TEST_PROGRAMS)
	@mkdir -p $(@D)
	$(CC) -o $@ $(foreach source,$(FRAMEWORK_SOURCES),$(call test_object,$(source))) $(call test_object,$(TEST_DIR)/shared/test_helpers.c) $(call test_object,$(ADAPTER_DIR)/group7_adapter_chat_smoke.c) $(LIB) $(LDFLAGS) $(LIBS)
...
TEST_RUN_LIST := $(FRAMEWORK_SELF_BIN) $(CONFORMANCE_BIN) $(ADAPTER_BINS)
...
_bench: $(ADAPTER_GROUP2_BINS)
	@SIN_EXTENDED_BENCH=1 SIN_BENCH_REPORT=1 ./$(OBJ_DIR)/$(ADAPTER_DIR)/test_runtime_benchmark --run test.runtime.test_runtime_benchmark_optin
