...
Secondly, Sinistra is released under the MIT license, and contributions must be made under the same license.

Thirdly, Sinistra development uses LLMs (mostly Codex at the moment) as a development partner.  AI contributions are accepted, but they must be a) clearly bounded, b) come with a detailed commit message, and c) must be understandable.  The last point is *important*: all PRs will be scrutinised carefully, and if the reviewer cannot understand *what* the PR actually does, or *why* it is done in a particular way, then the PR will not be merged.  In short, if you do not fully comprehend the minute details of your PR, whether it is generated by humans or by LLMs, then do not expect it to be accepted.
