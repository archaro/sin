...
static void print_raw(uint8_t *start, uint8_t *end) {
  if (!opt_raw || opt_quiet || opt_json) return;
  outln(" [raw:");
...
void usage() {
  ...
  logmsg("     --json\t\tMachine-readable JSON lines output.\n");
}
...
int main(int argc, char **argv) {
  ...
  while (opcodeptr < end && *opcodeptr != 'h') {
    if (!opt_json) outln("Byte %05u: ", opcodeptr - bytecode - 1);
  ...
  if (decode_failed) {
    ...
    if (opt_json) outln("{\"byte\":%u,\"op\":\"HALT\"}\n", (unsigned int)(opcodeptr - bytecode - 1));
...
uint8_t *decode_opcode(uint8_t *opcodeptr, uint8_t *end, int context) {
  ...
  if (opt_raw && !opt_quiet && !opt_json) outln("\n");
  return opcodeptr;
}
