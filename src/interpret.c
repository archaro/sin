...
uint8_t *op_inclocal(uint8_t *nextop, ITEM_t *item) {
  ...
  if (item->stack.stack[index].type == VALUE_int) {
    item->stack.stack[index].i++;
  } else {
...
uint8_t *op_savelocal(uint8_t *nextop, ITEM_t *item) {
  ...
  memcpy(&(item->stack.stack[index]),
                  &(item->stack.stack[item->stack.current]), sizeof(VALUE_t));
...
uint8_t *op_getlocal(uint8_t *nextop, ITEM_t *item) {
  ...
  memcpy(&(item->stack.stack[item->stack.current]),
                                &(item->stack.stack[index]), sizeof(VALUE_t));
#ifdef DEBUG
  VALUE_t v;
  v = peek_stack(&item->stack);
#endif
...
uint8_t *op_add(uint8_t *nextop, ITEM_t *item) {
  ...
  VALUE_t v1, v2;
  v1 = pop_stack(&item->stack);
  v2 = pop_stack(&item->stack);
  if (v1.type == VALUE_int && v2.type == VALUE_int) {
    v2.i += v1.i;
    v2.type = VALUE_int;
    push_stack(&item->stack, v2);
    ...
    push_stack(&item->stack, VALUE_NIL);
  }
  DEBUG_LOG("OP_ADD: types %d and %d\n", v1.type, v2.type);
  return nextop;
}

uint8_t *op_subtractint(uint8_t *nextop, ITEM_t *item) {
...
uint8_t *op_divideint(uint8_t *nextop, ITEM_t *item) {
...
uint8_t *op_multiplyint(uint8_t *nextop, ITEM_t *item) {
...
uint8_t *op_negateint(uint8_t *nextop, ITEM_t *item) {
  // If the top value on the stack is an int, negate it.
  //  Complain bitterly if not.
  if (item->stack.stack[item->stack.current].type == VALUE_int) {
    item->stack.stack[item->stack.current].i = -item->stack.stack[item->stack.current].i;
  } else {
    logerr("Attempt to negate a value of type '%d'.\n",
                                    item->stack.stack[item->stack.current].type);
  }
...
uint8_t *op_logicalnot(uint8_t *nextop, ITEM_t *item) {
  ...
  switch (item->stack.stack[item->stack.current].type) {
    case VALUE_bool:
      item->stack.stack[item->stack.current].i = !(item->stack.stack[item->stack.current].i);
      break;
    case VALUE_int:
      // If the int value is nonzero, then false, else true.
      item->stack.stack[item->stack.current].type = VALUE_bool;
      if (item->stack.stack[item->stack.current].i) {
        item->stack.stack[item->stack.current].i = 0;
      } else {
        item->stack.stack[item->stack.current].i = 1;
      }
      break;
    case VALUE_nil:
    ...
    case VALUE_str:
      ...
      FREE_ARRAY(char, item->stack.stack[item->stack.current].s,
                      strlen(item->stack.stack[item->stack.current].s) + 1);
      item->stack.stack[item->stack.current].type = VALUE_bool;
      item->stack.stack[item->stack.current].i = 0;
      break;
  }
...
uint8_t *op_logicaland(uint8_t *nextop, ITEM_t *item) {
  ...
  VALUE_t v1 = convert_to_bool(pop_stack(&item->stack));
  VALUE_t v2 = convert_to_bool(pop_stack(&item->stack));
