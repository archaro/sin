...
static uv_close_cb last_close_cb; static uv_write_cb last_write_cb; static uv_write_t *last_write_req;
...
void test_on_new_connection_rejections_and_close_ownership(void){reset_faults();config.maxconns=1;line=calloc(1,sizeof(*line));line[0].status=LINE_idle;on_new_connection(NULL,0);ASSERT_EQ_INT(1,uv_close_calls);free(line);line=NULL;reset_faults();config.maxconns=1;line=calloc(1,sizeof(*line));fail_malloc_after=0;on_new_connection(NULL,0);ASSERT_EQ_INT(0,uv_close_calls);free(line);line=NULL;reset_faults();config.maxconns=1;line=calloc(1,sizeof(*line));stub_telnet_init_fail=1;on_new_connection(NULL,0);ASSERT_EQ_INT(1,uv_close_calls);cleanup_lines();reset_faults();config.maxconns=1;line=calloc(1,sizeof(*line));stub_uv_read_start_result=UV_EIO;on_new_connection(NULL,0);ASSERT_EQ_INT(1,uv_close_calls);cleanup_lines();reset_faults();config.maxconns=1;line=calloc(1,sizeof(*line));stub_uv_accept_result=UV_ECONNRESET;on_new_connection(NULL,0);ASSERT_EQ_INT(1,uv_close_calls);free(line);line=NULL;reset_faults();config.maxconns=1;line=calloc(1,sizeof(*line));uv_tcp_t*orphan=calloc(1,sizeof(*orphan));client_on_close((uv_handle_t*)orphan);free(line);line=NULL;}
